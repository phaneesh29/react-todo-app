...
const App = () => {
  ...
  return (
    <>
      <ToastContainer />
      <div className='bg-slate-600 rounded-lg w-[500px] min-h-[600px] m-auto mt-10'>
        <form onSubmit={(e) => handleFormSubmit(e)} className='border-b-4 border-b-orange-400 flex flex-col gap-4 p-7 bg-emerald-800 justify-center rounded-lg'>
          <input type="text" name="task" value={title} onChange={(e) => handleTitle(e)} className='p-3 rounded-lg bg-transparent ring-4 ring-orange-800 focus:outline-none  text-2xl placeholder:text-2xl placeholder:text-gray-400' placeholder='Enter Task...' />
          <textarea name="desc" value={desc} onChange={(e) => handleDesc(e)} className='bg-transparent ring-4 ring-orange-700 rounded-lg  p-3 min-h-[100px] resize-y max-h-[200px] focus:outline-none text-2xl placeholder:text-2xl placeholder:text-gray-400' placeholder='Enter Description....'></textarea>
          <select name="category" value={selectedOption} onChange={handleSelect} className='p-3 rounded-lg bg-transparent text-2xl ring-4 ring-blue-400 focus:outline-none '>
          ...
          <button type="submit" className='p-3 bg-emerald-400 text-black rounded-lg text-xl font-semibold w-[300px] m-auto hover:bg-emerald-500 active:scale-75 transition-all'>Add Task</button>
        </form>
        ...
        <ul className='rounded-lg p-3 bg-stone-900 flex flex-col gap-4 text-xl mb-7'>
          {
            (taskArr.filter((t) => (
              categoryChange ? t.category === categoryChange : true
            ))).map((t, index) => (
              <li key={index} className={` p-2 px-4 text-xl rounded-lg flex items-center justify-between ${t.isCompleted ? "bg-green-800 line-through" : "bg-blue-950"} `}>
